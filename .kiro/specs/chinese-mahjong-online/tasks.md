...
- [x] 23. v1.3.1 区域隔离与稳定性修复
  - [x] 23.1 门前区与弃牌区隔离
    - 碰/杠 meld 信息锁定在玩家卡片旁（边缘区域），不进入绿色桌面
    - 弃牌矩阵严格在绿色桌面内，形成向心矩形结构
    - Grid 区域命名：top-e/top-r/left/ctr/right/bot-r
    - _需求: 22.1, 22.2_

  - [x] 23.2 积分面板遮挡修复
    - 积分面板定位改为 bottom: calc(4rem + 8px)，位于手牌区上方
    - 手牌容器 z-index: 50，积分面板 z-index: 30
    - 响应式断点下动态调整 bottom 偏移
    - _需求: 22 (Task 3)_

  - [x] 23.3 重连后积分看板恢复
    - onStarted 中补充 prevScoresRef 快照
    - scoreLog 在重连/dissolve 时不清空
    - _需求: 11.3, 18.6_
...
- [x] 25. v1.4.0-v1.4.1 牌面图形化
  - [x] 25.1 SVG 牌面渲染（v1.4.0，已被 PNG 替代）
  - [x] 25.2 区域隔离与 z-index 规范（v1.4.1）
    - 手牌 z-50，积分 z-40，碰杠 z-30，弃牌 z-20，背景 z-10
    - _需求: 22_

- [x] 26. v1.5.0-v1.5.2 资产集成与持久化身份
  - [x] 26.1 集成 riichi-mahjong-tiles PNG 牌面（CC0）
    - 35 张 PNG 复制到 public/tiles/，Tile.tsx 改为 img 渲染
    - _需求: 22.3_
  - [x] 26.2 集成 mahjong-master 音效（MIT）
    - 3 个 MP3 到 public/sounds/，audioService 接入真实音频
    - _需求: 22.6_
  - [x] 26.3 持久化玩家身份（localStorage + Socket auth）
    - _需求: 26.1, 26.2, 26.3, 26.4_
  - [x] 26.4 随机昵称 + 邀请链接 + socket.id→playerId 全面审查
    - _需求: 27, 28_
  - [x] 26.5 碰杠来源背面标记（替代横放）
    - 上家→左背面，下家→右背面，对家→中背面
    - _需求: 22.3_

- [x] 27. v1.6.0 核心功能升级
  - [x] 27.1 房间号 6→4 位 + 碰撞检查
    - _需求: 23_
  - [x] 27.2 昵称系统（编辑/持久化/状态锁/服务端映射）
    - _需求: 27_
  - [x] 27.3 房间状态清理（cleanupPlayerFromOldRoom）
    - _需求: 10_
  - [x] 27.4 积分面板可拖动悬浮
    - _需求: 18.6_
  - [x] 27.5 复制按钮 fallback
    - _需求: 28_
  - [x] 27.6 上庄逻辑修复（winnerIndex = currentPlayerIndex）
    - _需求: 10.8_
  - [x] 27.7 Compass 中心方位块
    - _需求: 29_

### V1.7.1 UI Bugfix

- [x] 28. 移动端手牌溢出修复
  - [x] 28.1 手牌 14 张时最右侧显示不全
    - G-bot grid area 改为跨三列（bot bot bot），手牌占满视口宽度
    - G-hand 添加 overflow-x:auto（隐藏滚动条），宽度不足时可横向滚动
    - 480px 断点下 tile-lg 从 100vw/17 缩小为 100vw/19
    - _需求: 20.2, 20.3, 20.4_
  - [x] 28.2 下家信息显示不全
...
- [ ] 31. 胡牌算法增强
  - [ ] 28.1 向听数计算（参考 mahjong-master shanten.js）
  - [ ] 28.2 AI 托管策略优化（基于向听数的最优弃牌）
  - [ ] 28.3 听牌提示（显示可胡的牌）

- [ ] 29. 动态牌墙视觉
  - [ ] 29.1 四方牌墙组件（17 墩 × 2 层）
  - [ ] 29.2 摸牌时牌墙动态递减动画
  - [ ] 29.3 弃牌区旋转策略（左家 90°/右家 -90°/对家 180°）

- [ ] 30. 用户系统增强
  - [ ] 30.1 昵称服务端 Redis 持久化（当前仅内存）
  - [ ] 30.2 历史战绩查询
  - [ ] 30.3 好友系统
